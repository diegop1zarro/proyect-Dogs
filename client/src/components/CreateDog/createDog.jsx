...
function validate(input) {
  ...
  if (!/[A-Z]/.test(input.name)) {
    ...
    errors.name =
      "You must put a name with its first letter in capital letters";
  } else if (!/^[A-Za-z0-9-ñ\s]+$/g.test(input.life_span)) {
    errors.life_span =
      "life span is required (can only contain numbers and letters)";
  ...
  } else if (eight(input.height) === true) {
    // cualquier numero y guiones
  ...
  return (
    <div>
      ...
      <form className="contenedorMenor" onSubmit={(e) => handleSubmit(e)}>
        ...
        <div>
          ...
          <select onChange={(e) => handleSelect(e)}>
            {Alltemperaments.map((Eltemperamento, i) => (
              <option key={i} value={Eltemperamento.name}>
                {Eltemperamento.name}
              </option>
            ))}
          </select>
        ...
        {errors.temperament && (
          <p className="errorCreate">{errors.temperament}</p>
        )}
        ...
        <button className="button_Create" type="submit">Create Breed</button>
      </form>
    </div>
  );
}
